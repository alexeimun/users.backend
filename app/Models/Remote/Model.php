...
abstract class Model implements ArrayAccess, Arrayable, Jsonable, JsonSerializable {
    ...
    public function getObservableEvents() {
        return ['creating', 'created', 'updating', 'updated', 'deleting', 'deleted', 'saving', 'saved',];
    }
    ...
    protected function makeRequest($endpoint = null, $method, $params) {
        ...
        if(static::getParentID()) {
            $params = array_merge([static::getParentID()], $params);
        }
    ...
    protected function performUpdate() {
        ...
        // Send updates
        $results = $this->makeRequest($this->getEndpoint(), 'update', [$this->getKey(), $params]);
        if($results) {
    ...
    protected function setKeysForSave() {
        ...
        $params = array_map(function ($value) {
            if(is_object($value) && !($value instanceof UploadedFile) && is_callable([$value, 'toApi'])) {
                return $value->toApi();
            }
            return $value;
        }, $this->toApi());
        // Remove dynamic params
        return array_except($params, array_merge([static::CREATED_AT, static::UPDATED_AT], ['id', 'pagination',]));
    }
    ...
    protected function hasToArray($value) {
        return (is_object($value) && !($value instanceof UploadedFile) && is_callable([$value, 'toArray']));
    }
    ...
    public static function paginate($method, array $params = []) {
        // Set request params
        $params = array_filter(array_merge(['page' => 1], $params));
        $instance = new static([], static::getParentID());;
        // Make request
    ...
    protected function performDeleteOnModel() {
        $results = $this->makeRequest($this->getEndpoint(), 'destroy', [$this->getKey()]);
        // Creation failed
        if(!$results) {
            return false;
        }
    ...
    protected function hasToApi($value) {
        return (is_object($value) && !($value instanceof UploadedFile) && is_callable([$value, 'toApi']));
    }
}